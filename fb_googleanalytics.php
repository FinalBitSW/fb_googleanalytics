...
class Fb_Googleanalytics extends Module
{
    ...
    public function hookdisplayHeader($params, $back_office = false)
    {
        if (Configuration::get('GA_ACCOUNT_ID')) {
            ...
            return $this->display(__FILE__, 'fb_googleanalytics.tpl');
        }
    }
            ...
            if ($ga_order_sent === false) {
                ...
                if ($order->id_customer == $this->context->cookie->id_customer) {
                    ...
                    $transaction = array(
                        ...
                        'shipping' => $order->total_shipping,
                        'tax' => $order->total_paid_tax_incl - $order->total_paid_tax_excl,
                        'url' => $this->context->link->getModuleLink('fb_googleanalytics', 'ajax', array(), true),
                        'customer' => $order->id_customer);
